...
class GraphSAGESkipNet(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels=64, num_layers=2, dropout=0.3, ratio=0.8, use_pos_enc=False):
        super().__init__()
        self.convs = torch.nn.ModuleList()
        ...
        # TopK pooling layer before global pooling
        self.pool = TopKPooling(hidden_channels, ratio=ratio)  # you can adjust the ratio as needed (thanks ChatGPT!)
