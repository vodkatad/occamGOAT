...
if __name__ == '__main__':
    ...
    dataset = CrabGOAT_classes.Graph_data(root=dir, edge_index=edge_index, gene_to_idx=gene_to_idx, wt=wt, kras=kras)
    ...
    if args.model == 'GAT':
        ...
        model = CrabGOAT_classes.GraphTransformerNet(
            in_channels=dataset.num_node_features,
            hidden_channels=hidden_channels,
            heads=3,
            num_layers=2,
    ...
    for epoch in range(1, num_epochs + 1):
        # Train step
        tr_loss, tr_acc = CrabGOAT_classes.train(model, train_loader, optimizer, criterion, device)
        # Validation step
